...
- **Coloured weather icons and a logo of your own.** The P4 panels draw the weather on the forecast and clock cards with ten PNG icons and show a logo above the words of the starting screen; `WEATHER_ICON_DIR`, `BOOT_LOGO` and `BOOT_LOGO_SIZE` in a screen's own YAML point them at yours (`images/no-logo.png` for no logo). A condition without an icon keeps its MDI glyph.
